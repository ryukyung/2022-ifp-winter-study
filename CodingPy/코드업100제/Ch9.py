...
# 문제 직사각형을 만드는 데 필요한 4개의 점 중 3개의 좌표가 주어질 때, 나머지 한 점의 좌표를 구하려고 합니다. 점 3개의 좌표가 들어있는 배열 v가 매개변수로 주어질 때, 
...
#솔루션 : x좌표가 겹치는것을 제하고, 남은 하나를 구하고. y좌표가 겹치는것을 제하고 남은 하나를 구하면 그것이 사각형의 마지막 포인트이다.
...
def solution():
    ...
    tempY = []
    #xor을 이용하여 x좌표 찾기
    tempX = (v[0][0] ^ v[1][0] ^ v[2][0])
    tempY = (v[0][1] ^ v[1][1] ^ v[2][1])
